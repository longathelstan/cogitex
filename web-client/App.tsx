    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800">
      ...
      <nav className="fixed top-0 left-0 right-0 bg-white/80 backdrop-blur-md border-b border-slate-200 z-30 h-16 flex items-center px-4 lg:px-8 justify-between">
        ...
        <div className="hidden lg:flex items-center gap-2 bg-slate-100/50 p-1.5 rounded-full border border-slate-200/50">
          <NavItem view={AppView.DASHBOARD} icon={<LayoutDashboard size={18} />} label="Bảng điều khiển" />
          <NavItem view={AppView.MIND_MIRROR} icon={<Brain size={18} />} label="Gương tư duy" />
          <NavItem view={AppView.SKILL_QUEST} icon={<Gamepad2 size={18} />} label="Nhiệm vụ kỹ năng" />
          <NavItem view={AppView.RISE_BOARD} icon={<Users size={18} />} label="Cộng đồng" />
          <NavItem view={AppView.LEARN_PULSE} icon={<BarChart3 size={18} />} label="Phân tích" />
        </div>

        <div className="flex items-center gap-4">
          <div className="hidden lg:block text-right">
            <p className="text-xs font-bold text-slate-700">{user.name}</p>
            <p className="text-[10px] text-slate-400 font-medium">Học giả Cấp 3</p>
          </div>
          <img
